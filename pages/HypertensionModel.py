# pages/04HypertensionModel.py
import streamlit as st
import numpy as np
from utils.predictions import load_model, predict_score
from utils.preprocessing import load_scaler_hypertension
...
def run_hypertension_risk_prediction():
    st.write("Running the Hypertension Risk Prediction model...")
    # Your prediction model code goes here
    st.write("Prediction: Hypertension Risk Level")
# Setup session state for navigation
if 'hypertension_form_page' not in st.session_state:
    st.session_state.hypertension_form_page = "non_tech"

st.title("Hypertension Risk Prediction")

if st.session_state.hypertension_form_page == "non_tech":
    ...
    if col2.button("Next ➡️"):
        st.session_state.hypertension_form_page = "tech"
        st.rerun()

elif st.session_state.hypertension_form_page == "tech":
    ...
    if col2.button("Calculate Hypertension Risk"):
        feature_columns = [
        ...
        # Feedback
        if final_score >= 85:
            category = "Excellent: Your blood pressure is well controlled."
        ...
        st.success(f"**Hypertension Control Score**: {final_score:.2f}")
        st.info(category)
